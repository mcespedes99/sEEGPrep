...
def interpolate_bads(inst, picks, dots=None, reset_bads=True, mode='accurate'):
    ...
    import mne
    eeg_picks = set(pick_types(inst.info, meg=False, eeg=True, seeg=False, exclude=[]))
    eeg_picks_interp = [p for p in picks if p in eeg_picks]
    meg_picks = set(pick_types(inst.info, meg=True, eeg=False, seeg=False, exclude=[]))
    meg_picks_interp = [p for p in picks if p in meg_picks]
    seeg_picks = set(pick_types(inst.info, meg=False, eeg=False, seeg=True, exclude=[]))
    seeg_picks_interp = [p for p in picks if p in seeg_picks]
    ...
    with mne.utils.use_log_level('error'):
        ...
        if len(meg_picks_interp) > 0:
            _interpolate_bads_meg_fast(inst, picks=meg_picks_interp,
                                       dots=dots, mode=mode)
        if len(seeg_picks_interp) > 0:
            _interpolate_bads_seeg(inst, picks=seeg_picks_interp)
    if reset_bads is True:
        inst.info['bads'] = []
    return inst

def _interpolate_bads_seeg(inst, picks=None):
    """ Interpolate bad sEEG channels.
    ...
    picks : str | list | slice | None
        ...
        None (default) will pick all sEEG channels. Note that channels in
    ...
    # Returns pick of all channels and ensures channels are correctly ordered
    ...
    ch_names = [inst.info['ch_names'][p] for p in picks]
    bads_chns = [ch for ch in inst.info['bads'] if ch in ch_names]
    if len(bads_chns) == 0:
        return
    picks_bad = pick_channels(inst.info['ch_names'], bads_chns, exclude=[])
    bads_mask = [p in picks_bad for p in picks]

    chs = [inst.info['chs'][i] for i in picks]
    locs3d = np.array([ch['loc'][:3] for ch in chs])
